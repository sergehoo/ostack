...
export const commands: Record<string, { description: string; handler: CommandHandler }> = {
  ...
  security: { description: "Audit de sécurité défensif (review, dependencies, threat-model, catalog, permissions, containers, evidence, retest)", handler: async (context) => (await import("./security.js")).runSecurity(context) },
  incident: { description: "Squelette de réponse à incident de sécurité (détecter, contenir, éradiquer, restaurer, capitaliser)", handler: async (context) => (await import("./security.js")).runIncident(context) },
